...
void SnappersBlend_float(in float2 uv, in float3 inAlbedo, in float4 inNormal, in float inCavity, out float3 outAlbedo, out float4 outNormal, out float outCavity, out float blendAmount)
{
	...
	blendAmount = 1 - ((1 - AddOpAll_upperbound_1) + IntersectionInv * AddOpAll * 0.5 ) ;
	// blend albedo
	{
#ifdef _SNAPPERS_TEXTURE_ARRAYS
